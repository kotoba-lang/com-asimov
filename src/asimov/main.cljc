(ns asimov.main
  "Kotodama WASM entrypoint for the Asimov clean-room actor (L4) — Clojure port.
  ...
  facts (`asimov.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "asimov")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"asimov.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "asimov-compat" :tier tier :entities entities} 200])
...
(defn start! [] :asimov-compat/ready)
